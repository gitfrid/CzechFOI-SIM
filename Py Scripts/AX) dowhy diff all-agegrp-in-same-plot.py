...
plot_title_text="Difference of D-curves and difference of Doses-curves for AG 1 year appart, for age groups from 15 to 85" # Title of plot
annotation_text ="AX) dowhy diff all-agegrp-in-same-plot.py / diff D_Curves, diff Doses_curves without dAEFIS (RAW) -vs- diff D_Curves, diff Doses_curves incl. simulated dAEFIs (AEF) / not normalized "
plotfile_name = "AX) dowhy diff all-agegrp-in-same-plot" 
...
def main():
    ...
    dowhy_trace_pairs_raw = [
        (f'Avg DIF-NUM_VDA', f'Avg DIF-NUM_D')
    ]
    ...
    layout = go.Layout(
        title="Causal Impact Comparison Doses_Curve -> D_Curve\n(diff RAW vs. diff AEF=simulated dAEFI - AG 1 year appart)",
        xaxis=dict(title="diff Doses_curve / Age"),
        yaxis=dict(title="diff D_Curve / diff Doses per one dAEF"),
        showlegend=True        
    )
...
def plot_causalimpact_for_traces(trace_manager, dowhy_trace_pairs, age_band_compare, html_file_path, plot_data=None):
    ...
    name3_prefix = f"{dowhy_trace_pairs[0][0]} AEF"
    name4_prefix = f"{dowhy_trace_pairs[0][1]} AEF"
    ...
    for idx, (start, end) in enumerate(age_band_compare):
        print(f"Processing Age Band {start} and {end}")    
        ...
        try:
            ...
            matching_name3 = [key for key in y_series.keys() if key.startswith(name3_prefix) and key.endswith(age_band_extension)]
            matching_name4 = [key for key in y_series.keys() if key.startswith(name4_prefix) and key.endswith(age_band_extension)]
        ...
        trace = go.Scatter(
            ...
            mode='lines+markers',
            name=f"{name1_prefix}-{name2_prefix} Age Band {start}",
        ...
        trace = go.Scatter(
            ...
            mode='lines+markers',
            name=f"{name3_prefix}-{name4_prefix} Age Band {start}",
