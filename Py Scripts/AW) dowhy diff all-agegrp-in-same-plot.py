...
annotation_text ="AW) dowhy all-agegrp-in-same-plot.py / D_Curves without dAEFIS (RAW) -vs- D_Curves with simulated dAEFIs (AEF)/ not normalized "
plotfile_name = "AW) dowhy diff all-agegrp-in-same-plot" 
...
def main():
    ...
    layout = go.Layout(
        title="Causal Impact Comparison Doses_Curve -> D_Curve \n(RAW vs. AEF=simulated dAEFIs - AG 1 year appart)",
        xaxis=dict(title="Doses_curve / Age"),
        yaxis=dict(title="D_Curve / Doses per one dAEF"),
        showlegend=True
    )
...
def plot_causalimpact_for_traces(trace_manager, dowhy_trace_pairs, age_band_compare, html_file_path, plot_data=None):
    ...
    doses_per_death_diff_trace = go.Scatter(
        ...
        mode='markers',
        name='Doses per Death difference (AEF-RAW)',
