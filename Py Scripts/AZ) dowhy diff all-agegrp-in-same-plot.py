...
annotation_text ="AZ) dowhy diff all-agegrp-in-same-plot.py / diff D_Curves, diff Doses_curves RAW -vs- diff D_Curves, diff Doses_curves + 1 year / not normalized "
plotfile_name = "AZ) dowhy diff all-agegrp-in-same-plot" 
...
def main():
    ...
    layout = go.Layout(
        title="Causal Impact Comparison differnece raw Doses_Curve -> differnece raw D_Curve\n(diff RAW vs. diff RAW + 1 year AG)",
...
def plot_causalimpact_for_traces(trace_manager, dowhy_trace_pairs, age_band_compare, html_file_path, plot_data=None):
    ...
    for idx, (start, end) in enumerate(age_band_compare):
        ...
        start_int = int(start) + 1   # Cast start to an integer
        end_int = int(end) + 1      # Cast end to an integer
        age_band_extension2 = f"{start_int}<br>{end_int}"
        ...
        for k in range(2):
            ...
            if k == 1:
                age_group_text = f"Age Group: {start_int}-{end_int}\nMean Doses: {mean_doses:.6f}\nMean D Curve: {mean_d_curve:.6f}\nCausal Estimate: {causal_effect_value:.8f}"
        ...
        trace = go.Scatter(
            ...
            mode='lines+markers',
            name=f"{name1_prefix}-{name2_prefix} {start}-{end}",
        ...
        trace = go.Scatter(
            ...
            mode='lines+markers',
            name=f"{name3_prefix}-{name4_prefix}+1 {start_int}-{end_int}",
    ...
    doses_per_death_trace = go.Scatter(
        ...
        mode='markers',
        name='causal estimate (diff RAW+1)',
    ...
    doses_per_death_diff_trace = go.Scatter(
        ...
        mode='markers',
        name='Doses per Death Difference (diff RAW+1 - diff RAW)',
