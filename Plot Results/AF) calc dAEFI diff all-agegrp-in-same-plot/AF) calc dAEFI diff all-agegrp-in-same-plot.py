...
# The goal is to find a reliable method for backcalculating the estimated dAEFIs 
...
plot_title_text="Sum of the differences between age groups of raw D-curves and doses, calculated for age groups from 15 to 85, including rolling and shift correlation" # Title of plot
annotation_text ="AF) calc dAEFI diff all-agegrp-in-same-plot.py / Curves without dAEFIS -vs- Curves with simulated dAEFIs / D-curves raw data (not normalized) "
plotfile_name = "AF) calc dAEFI diff all-agegrp-in-same-plot" 
...
def main():
    ...
    for k in range(2):  
        ...
        if calc_correlation:    
            ...
            corr_band_pairs = [
                    (f'Avg sum DIF-NUM_VDA {dae}', f'Avg sum DIF-NUM_D {dae}')
            ]    
            plot_rolling_correlation_and_phase_shift_for_traces(trace_manager, corr_band_pairs,"", colors, 10, max_phase_shift) 

            # Loop through age band list and calc correlation for each age band trace 
            corr_band_pairs = [
                (f'Avg DIF-NUM_VDA {dae}', f'Avg DIF-NUM_D {dae}')
            ]    
            n_color=0
            for start, end in age_band_compare:
                print(f"{start} {end}")    
                n_color += 1
                plot_rolling_correlation_and_phase_shift_for_traces(trace_manager, corr_band_pairs,f"{start}<br>{end}", colors, n_color, max_phase_shift) 
