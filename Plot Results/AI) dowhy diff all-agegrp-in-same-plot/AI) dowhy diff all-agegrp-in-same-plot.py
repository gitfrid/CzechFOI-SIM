...
annotation_text ="AI) dowhy diff all-agegrp-in-same-plot.py / Curves without dAEFIS -vs- Curves with simulated dAEFIs / D-curves not normalized "
plotfile_name = "AI) dowhy diff all-agegrp-in-same-plot" 
...
future_day_range = 60          # Random future day range (for example 1 to 14)
...
def main():
    ...
    for k in range(2):
        ...
        # Call function for RAW traces
        dowhy_trace_pairs_raw = [
            (f'Avg DIF-NUM_VDA RAW', f'Avg DIF-NUM_D RAW')
        ]
        ...
        # Call function for AEF traces (with a reversed color palette for AEF)
        dowhy_trace_pairs_aef = [
            (f'Avg DIF-NUM_VDA AEF', f'Avg DIF-NUM_D AEF')
        ]
        plot_data_combined = plot_causalimpact_for_traces(trace_manager, dowhy_trace_pairs_aef, age_band_compare, html_file_path, plot_data=plot_data_combined)

        # Create the layout for the Plotly figure with a combined title
        layout = go.Layout(
            title="Causal Impact Comparison\nRAW vs. AEF",
            xaxis=dict(title="Doses_curve"),
            yaxis=dict(title="D_Curve"),
            showlegend=True
        )
...
def plot_causalimpact_for_traces(trace_manager, dowhy_trace_pairs, age_band_compare, html_file_path, plot_data=None):
    # This function now accepts an optional 'plot_data' argument to accumulate traces from both RAW and AEF.
    # It also calculates and plots the estimated mean values for each age group, grouped under two legend entries:
    # 'mean_estim_RAW' and 'mean_estim_AEF', and includes the age group in the hover text.

    # If no plot_data is provided, initialize an empty list
    ...
    name1_prefix = dowhy_trace_pairs[0][0]  # 'Avg DIF-NUM_VDA RAW' or 'Avg DIF-NUM_VDA AEF'
    name2_prefix = dowhy_trace_pairs[0][1]  # 'Avg DIF-NUM_D RAW' or 'Avg DIF-NUM_D AEF'
    ...
    for idx, (start, end) in enumerate(age_band_compare):
        print(f"{start} {end}")    
        age_band_extension = f"{start}<br>{end}"

        # Get the relevant traces based on age band
        try:
            ...
            if matching_name1 and matching_name2:
                Doses_curve = y_series[matching_name1[0]]
                D_Curve = y_series[matching_name2[0]]
            else:
                print(f"Warning: no matching traces found for :{name1_prefix}*{age_band_extension}:{name2_prefix}*{age_band_extension}: Skipping this age band.")
                continue  # Skip this pair if no match found

        except KeyError:
            print(f"Error: no matching traces found for :{name1_prefix}*{age_band_extension}:{name2_prefix}*{age_band_extension}:. Skipping this age band.")
            continue  # Skip this pair if no match found

        # --- Causal Inference with DoWhy ---
        print(f"Performing causal analysis for Age Band {start}-{end} using DoWhy...")

        # Prepare data as a pandas dataframe for DoWhy
        ts_data = pd.DataFrame({
            'Doses_curve': Doses_curve.values,
            'D_Curve': D_Curve.values,
        })

        # Define a causal model with a DAG (Directed Acyclic Graph)
        causal_graph = """
        digraph {
            Doses_curve -> D_Curve;
        }
        ...
        # Estimate the causal effect using a valid method (e.g., linear regression)
        ...
        age_group_text = f"Age Group: {start}-{end}\nMean Doses: {mean_doses:.2f}\nMean D Curve: {mean_d_curve:.2f}"

        # Determine color for mean values (based on RAW or AEF)
        if "AEF" in name1_prefix or "AEF" in name2_prefix:
            mean_aef_points.append((mean_doses, mean_d_curve))  # Add mean point to AEF list
            mean_aef_text.append(age_group_text)  # Add hover text for AEF
        else:
            mean_raw_points.append((mean_doses, mean_d_curve))  # Add mean point to RAW list
            mean_raw_text.append(age_group_text)  # Add hover text for RAW

        # Choose color based on whether we are dealing with RAW or AEF
        if "AEF" in name1_prefix or "AEF" in name2_prefix:
            colors = px.colors.qualitative.Plotly_r
        else:
            colors = px.colors.qualitative.Plotly

        # Get the color corresponding to the index (cycling through the color list)
        color = colors[(idx + 2) % len(colors)]  # Color cycling through the color palette

        # --- Plot the results using Plotly ---
        trace = go.Scatter(
            x=Doses_curve.values, 
            y=D_Curve.values, 
            mode='lines+markers',  # Use both lines and markers
            name=f"{name1_prefix}-{name2_prefix} Age Band {start}-{end}",
            line=dict(color=color, width=1),  # Small line width (1)
            marker=dict(color=color, size=3)  # Small marker size (3)
        )
        # Append the trace to the provided plot_data list
        plot_data.append(trace)

    # After processing all age bands, create mean traces for RAW and AEF
    mean_raw_trace = go.Scatter(
        x=[point[0] for point in mean_raw_points],  # Extract Doses_curve values for RAW means
        y=[point[1] for point in mean_raw_points],  # Extract D_Curve values for RAW means
        mode='markers',
        name='mean_estim_RAW',  # Common name for all RAW mean points
        marker=dict(color='blue', size=10, symbol='x'),
        text=mean_raw_text,  # Add hover text for RAW mean points
        hoverinfo='text'  # Only show hover text
    )

    mean_aef_trace = go.Scatter(
        x=[point[0] for point in mean_aef_points],  # Extract Doses_curve values for AEF means
        y=[point[1] for point in mean_aef_points],  # Extract D_Curve values for AEF means
        mode='markers',
        name='mean_estim_AEF',  # Common name for all AEF mean points
        marker=dict(color='red', size=10, symbol='x'),
        text=mean_aef_text,  # Add hover text for AEF mean points
        hoverinfo='text'  # Only show hover text
    )

    # Add the mean traces to the plot_data list
    plot_data.append(mean_raw_trace)
    plot_data.append(mean_aef_trace)

    # Return the accumulated plot_data
    return plot_data
